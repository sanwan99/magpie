...
const { SquidMascot, SplatBackdrop, SplatBurst } = window;
...
function App() {
  ...
  const isSplat = t.accent === 'splat';
  // Burst counter — incrementing key forces a fresh splat-burst each paste
  const [burstSeed, setBurstSeed] = React.useState(0);
  React.useEffect(() => { if (pasteFlash) setBurstSeed((s) => s + 1); }, [pasteFlash]);

  return (
    <>
      <Desktop />

      {isSplat && <SplatBackdrop />}

      {isSplat && (
        <>
          <SquidMascot side="left"  team="yellow" visible={panelVisible} seed={3} />
          <SquidMascot side="right" team="purple" visible={panelVisible} seed={7} />
        </>
      )}
      ...
      <div className={`panel-stage ${panelVisible ? 'is-up' : 'is-down'}`}>
        ...
        {pasteFlash && !isSplat && <PasteFlash />}
        {pasteFlash && isSplat && (
          <SplatBurst key={burstSeed} team={burstSeed % 2 ? 'purple' : 'yellow'} />
        )}
      </div>
